...
const PLANS = [
  { id:"trial", label:"Essai gratuit", price:"0€", duration:"30 jours", color:"#888" },
  { id:"starter", label:"Starter", price:"29€/mois", duration:"1 établissement", color:"#185FA5" },
  { id:"pro", label:"Pro", price:"59€/mois", duration:"Utilisateurs illimités", color:"#0F6E56" },
  { id:"multi", label:"Multi-sites", price:"149€/mois", duration:"5 établissements", color:"#534AB7" },
]
...
function SuperAdmin({ session, onLogout }) {
  ...
  const createClient = async () => {
    ...
    setSaving(true)
    const slug = newClient.name.toLowerCase().replace(/\s+/g, "-").replace(/[^a-z0-9-]/g, "")
  ...
  return (
    <div style={{ fontFamily:"'DM Sans','Trebuchet MS',sans-serif", maxWidth:900, margin:"0 auto", background:"#FAFAF8", minHeight:"100vh" }}>
      {/* Header */}
      <div style={{ background:"#1D9E75", padding:"16px 24px", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
        <div style={{ display:"flex", alignItems:"center", gap:12 }}>
          ...
          <div>
            ...
            <div style={{ fontSize:11, color:"rgba(255,255,255,0.7)" }}>Tableau de bord super-administrateur</div>
          </div>
        </div>
        <div style={{ display:"flex", alignItems:"center", gap:12 }}>
          <span style={{ fontSize:12, color:"rgba(255,255,255,0.8)" }}>{session.user.email}</span>
          <button onClick={onLogout} style={{ fontSize:11, padding:"6px 14px", background:"rgba(255,255,255,0.15)", color:"#fff", border:"1px solid rgba(255,255,255,0.3)", borderRadius:8, cursor:"pointer", fontFamily:"inherit" }}>
            Déconnexion
          </button>
        </div>
      </div>
      ...
      <div style={{ background:"#fff", borderBottom:"0.5px solid #E8E8E4", display:"flex", padding:"0 24px" }}>
        {NAV.map(n => <button key={n.id} onClick={() => setPage(n.id)} style={{ padding:"14px 20px", border:"none", borderBottom: page===n.id ? "2px solid #1D9E75" : "2px solid transparent", background:"transparent", cursor:"pointer", fontFamily:"inherit", fontSize:13, fontWeight: page===n.id ? 600 : 400, color: page===n.id ? "#1D9E75" : "#666", display:"flex", alignItems:"center", gap:6 }}>
      ...
      <div style={{ padding:24 }}>
          ...
          <div>
            <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:16, marginBottom:24 }}>
              {[
                ...
                { label:"En période d'essai", val: tenants.filter(t=>t.plan==="trial").length, icon:"⏳", color:"#BA7517", bg:"#FAEEDA" },
            ...
            <div style={{ display:"grid", gridTemplateColumns:"2fr 1fr", gap:16 }}>
              {/* Derniers clients */}
              <div style={{ background:"#fff", border:"0.5px solid #E8E8E4", borderRadius:12, padding:"20px" }}>
                ...
                {PLANS.map(p => {
                  ...
                  return <div key={p.id} style={{ marginBottom:14 }}>
                    ...
                    <div style={{ height:6, background:"#F0F0EC", borderRadius:3 }}>
                      <div style={{ height:"100%", background:p.color, borderRadius:3, width:`${pct}%`, transition:"width 0.3s" }}/>
                    </div>
                  </div>
                })}
              </div>
        ...
        {page === "clients" && (
          <div>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:20 }}>
              <div style={{ fontSize:16, fontWeight:700, color:"#222" }}>Gestion des clients ({tenants.length})</div>
              <button onClick={() => setShowNewClient(true)} style={{ padding:"10px 20px", background:"#1D9E75", color:"#fff", border:"none", borderRadius:10, cursor:"pointer", fontFamily:"inherit", fontSize:13, fontWeight:600 }}>
                + Nouveau client
              </button>
            </div>
            ...
            {showNewClient && <div style={{ background:"#fff", border:"1px solid #1D9E75", borderRadius:12, padding:"20px", marginBottom:20 }}>
              <div style={{ fontSize:14, fontWeight:600, color:"#222", marginBottom:16 }}>Créer un nouveau client</div>
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12, marginBottom:12 }}>
                {[
                  { key:"name", label:"Nom du restaurant *", placeholder:"Ex: Snack El Baraka" },
                  { key:"email", label:"Email *", placeholder:"contact@restaurant.com" },
                  { key:"phone", label:"Téléphone", placeholder:"+33 6 XX XX XX XX" },
                  { key:"address", label:"Adresse", placeholder:"123 rue de la Paix, Paris" },
                ].map(f => <div key={f.key}>
              ...
              <div style={{ display:"flex", gap:10 }}>
                <button onClick={createClient} disabled={saving} style={{ padding:"10px 24px", background:"#1D9E75", color:"#fff", border:"none", borderRadius:8, cursor:"pointer", fontFamily:"inherit", fontSize:13, fontWeight:600 }}>
                  {saving ? "Création..." : "Créer le client"}
                </button>
                <button onClick={() => { setShowNewClient(false); setMsg("") }} style={{ padding:"10px 24px", background:"#fff", color:"#666", border:"1px solid #E0E0DC", borderRadius:8, cursor:"pointer", fontFamily:"inherit", fontSize:13 }}>
                  Annuler
                </button>
              </div>
            </div>}
            ...
            <div style={{ background:"#fff", border:"0.5px solid #E8E8E4", borderRadius:12, overflow:"hidden" }}>
              <div style={{ display:"grid", gridTemplateColumns:"2fr 1fr 1fr 1fr 80px", padding:"12px 20px", background:"#F5F5F2", fontSize:11, fontWeight:600, color:"#888", textTransform:"uppercase", letterSpacing:"0.05em" }}>
                <div>Restaurant</div><div>Plan</div><div>Statut</div><div>Créé le</div><div>Actions</div>
              </div>
              {loading ? <div style={{ padding:20, color:"#888", fontSize:13 }}>Chargement...</div> :
                tenants.length === 0 ? <div style={{ padding:20, color:"#888", fontSize:13, textAlign:"center" }}>Aucun client pour l'instant</div> :
                tenants.map((t,i) => <div key={t.id} style={{ display:"grid", gridTemplateColumns:"2fr 1fr 1fr 1fr 80px", padding:"14px 20px", borderTop: i>0 ? "0.5px solid #F0F0EC" : "none", alignItems:"center" }}>
                  <div>
                    <div style={{ fontSize:13, fontWeight:600, color:"#222" }}>{t.name}</div>
                    <div style={{ fontSize:11, color:"#888" }}>{t.email}</div>
                  </div>
                  ...
                  <div>
                    <button onClick={() => toggleActive(t.id, t.is_active)} style={{ fontSize:11, padding:"4px 10px", background: t.is_active ? "#FCEBEB" : "#E1F5EE", color: t.is_active ? "#A32D2D" : "#0F6E56", border:"none", borderRadius:6, cursor:"pointer", fontFamily:"inherit" }}>
        ...
        {page === "stats" && (
          <div>
            <div style={{ fontSize:16, fontWeight:700, color:"#222", marginBottom:20 }}>Statistiques globales</div>
            <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:16 }}>
              {[
                { label:"Revenus mensuels estimés", val: `${tenants.filter(t=>t.plan==="starter").length*29 + tenants.filter(t=>t.plan==="pro").length*59 + tenants.filter(t=>t.plan==="multi").length*149}€`, icon:"💰", color:"#0F6E56", bg:"#E1F5EE" },
                { label:"Taux de conversion", val: tenants.length ? `${Math.round((tenants.filter(t=>t.plan!=="trial").length/tenants.length)*100)}%` : "0%", icon:"📈", color:"#185FA5", bg:"#E6F1FB" },
                { label:"Clients cette semaine", val: tenants.filter(t => new Date(t.created_at) > new Date(Date.now()-7*24*60*60*1000)).length, icon:"🆕", color:"#534AB7", bg:"#EEEDFE" },
              ].map((k,i) => <div key={i} style={{ background:"#fff", border:"0.5px solid #E8E8E4", borderRadius:12, padding:"20px" }}>
            ...
            <div style={{ background:"#fff", border:"0.5px solid #E8E8E4", borderRadius:12, padding:"20px", marginTop:16 }}>
              <div style={{ fontSize:14, fontWeight:600, color:"#222", marginBottom:16 }}>Détail par plan</div>
              {PLANS.map(p => {
                const count = tenants.filter(t => t.plan === p.id).length
                const revenue = p.id === "starter" ? count*29 : p.id === "pro" ? count*59 : p.id === "multi" ? count*149 : 0
                return <div key={p.id} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"12px 0", borderBottom:"0.5px solid #F0F0EC" }}>
                  ...
                  <div style={{ display:"flex", gap:24 }}>
                    ...
                    <span style={{ fontSize:13, fontWeight:600, color:"#222" }}>{revenue}€/mois</span>
                  </div>
                </div>
              })}
              <div style={{ display:"flex", justifyContent:"flex-end", marginTop:12, paddingTop:12, borderTop:"1px solid #E8E8E4" }}>
                <span style={{ fontSize:14, fontWeight:700, color:"#1D9E75" }}>
                  Total : {tenants.filter(t=>t.plan==="starter").length*29 + tenants.filter(t=>t.plan==="pro").length*59 + tenants.filter(t=>t.plan==="multi").length*149}€/mois
                </span>
              </div>
            </div>
          </div>
